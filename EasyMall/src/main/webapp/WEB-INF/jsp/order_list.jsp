				...
				支付状态：
					...
					<c:if test="${orderInfo.order.getstate == 0}">
						<a href="${ pageContext.request.contextPath }/order/payorder?id=${orderInfo.order.id}">
							...
							<c:if test="${orderInfo.order.paystate == 1}">
								<a href="${ pageContext.request.contextPath }/order/setstate?id=${orderInfo.order.id}">
									<font color="red">确定收货</font>
								</a>&nbsp;
							</c:if>
					...
					<c:if test="${orderInfo.order.sendstate == 1&&orderInfo.order.getstate == 0}">
